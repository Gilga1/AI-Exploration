...
def test_health_graph(client):
    res = client.get("/health/graph")
    assert res.status_code == 200
    assert "neo4j_connected" in res.json()


def test_health_warehouse(client):
    res = client.get("/health/warehouse")
    assert res.status_code == 200
    body = res.json()
    assert "snowflake_configured" in body
